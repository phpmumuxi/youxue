...
use app\shop\model\Analysis as AnalysisModel;
...
class Analysis extends AdminBase
{
    ...
    public function _initialize()
    {
        ...
        $ShopCommonModel = new ShopCommonModel();
        $shopId = $ShopCommonModel->getShopIdFromAdminId($adminId);
        $this->shopId = $shopId;
    }
    ...
    public function signCount()
    {
        ...
        $data = $AnalysisModel->getSignCount($this->shopId, $aTime, $bTime);
    ...
    public function signCountAjax()
    {
        ...
        if (input('?post.aTime') && input('?post.bTime')) {
            ...
            $AnalysisModel = new AnalysisModel();
            $retData = $AnalysisModel->getSignCount($this->shopId, $aTime, $timeB);
    ...
    public function signCountMonthAjax()
    {
        ...
        if (input('?post.aTime') && input('?post.bTime')) {
            ...
            $AnalysisModel = new AnalysisModel();
            $retData = $AnalysisModel->getSignCountMonth($this->shopId, $aTime, $timeB);
    ...
    public function orderCourseCountAjax()
    {
        ...
        if (input('?post.type')) {
            ...
            $resData = [];
            if ($type == 1 || $type == 2) {
                $resData = $AnalysisModel->getOrderCourseCountDay($this->shopId, $date['time']['timeA'], $date['time']['timeB']);
            }
            if ($type == 3) {
                $resData = $AnalysisModel->getOrderCourseCountMonth($this->shopId, $date['time']['timeA'], $date['time']['timeB']);
            }
            if ($resData) {
     ...
     * @return json
     */
    public function moneyCountAjax()
    {
        $res['status'] = 'false';
        if (input('?post.type')) {
            $type = input('post.type');
            ...
            $resData = [];
            if ($type == 1 || $type == 2) {
                $resData = $AnalysisModel->getSchoolMoneyDay($this->shopId, $date['time']['timeA'], $date['time']['timeB']);
            }
            if ($type == 3) {
                $resData = $AnalysisModel->getSchoolMoneyMonth($this->shopId, $date['time']['timeA'], $date['time']['timeB']);
            }
            if ($resData) {
                foreach ($date['date'] as $k => $v) {
                    $fillData[$k] = 0;
    ...
    public function courseBuyAjaxObsolete()
    {
        ...
        if (input('?post.type')) {
            ...
            $resData = [];
            if ($type == 1 || $type == 2) {
                $resData = $AnalysisModel->getCourseBuyDay($this->shopId, $date['time']['timeA'], $date['time']['timeB']);
            }
            if ($type == 3) {
                $resData = $AnalysisModel->getCourseBuyMonth($this->shopId, $date['time']['timeA'], $date['time']['timeB']);
            }
            if ($resData) {
                foreach ($date['date'] as $k => $v) {
                    $fillData[$k] = 0;
